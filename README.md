...
> **MINSPLICE**
  Is the minimal slicelength that wil be generated by the tool without generating warning messages.   Mosaic strongly advises to maintain a minimal slice length of 80mm.   If short splices are detected use the methods described in the usage section to resolve before sending the file to the printer!  Printing with shorter splices may cause unwanted behaviour (splice being reversed in the splice tube, palette 2 not being able to generate sufficient amount of filament to keep the buffer going).
...
The P2PP ENDPRUGETOWER has to be the very first line in the filament description.
```
;P2PP ENDPURGETOWER
